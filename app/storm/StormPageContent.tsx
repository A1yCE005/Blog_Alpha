    ...
    (node: HTMLDivElement) => {
      ...
      const step = (timestamp: number) => {
        ...
        if (progress >= 1 || remaining <= 0.35) {
          const latestTarget = resolveTargetScrollTop(node, host);
          host.scrollTop = latestTarget;
          finish();
          return;
        }
...
function StormQuoteCard({
  ...
  const textSpanClasses =
    "inline-block whitespace-pre-line overflow-visible transition-all duration-500 ease-out will-change-filter will-change-transform";
