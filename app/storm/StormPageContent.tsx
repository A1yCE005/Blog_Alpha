...
export function StormPageContent({ quotes }: StormPageContentProps) {
  ...
  const finishProgrammaticScroll = useCallback((after?: () => void) => {
  ...
  const resolveTargetScrollTop = useCallback((
    ...
    if (current !== container) {
      ...
      offset = container.scrollTop + (nodeRect.top - containerRect.top);
    }

    return offset + node.offsetHeight / 2 - container.clientHeight / 2;
  }, []);
  ...
  const animateContainerToNode = useCallback(
    (node: HTMLDivElement) => {
      ...
      cancelScrollAnimation({ skipFinish: true });
  ...
  const handleScroll = useCallback(
    (event: UIEvent<HTMLDivElement>) => {
      ...
      if (suppressScrollHandlingRef.current || programmaticScrollRef.current) {
        return;
      }
...
function StormQuoteCard({
  ...
  const baseTextClasses =
    "max-w-3xl whitespace-pre-line text-center text-[1.35rem] font-normal leading-relaxed transition-all duration-500 ease-out sm:text-[1.7rem] md:text-[1.9rem]";

  const highlightedClasses = highlighted
    ? "text-violet-300 storm-quote-highlight"
    : depthActive
    ? "text-zinc-500/60 blur-[5px] opacity-[0.3] saturate-[0.35]"
    : "text-zinc-200";
