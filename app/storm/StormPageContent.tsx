...
export function StormPageContent({ quotes }: StormPageContentProps) {
  ...
  const resolveTargetScrollTop = useCallback(
    (node: HTMLDivElement, container: HTMLDivElement) => {
      ...
      return container.scrollTop + (nodeCenter - containerCenter);
    },
    []
  );
  ...
  const applyHighlightToCenter = useCallback(
    (options?: { animate?: boolean }) => {
      ...
      const shouldAnimate = Boolean(options?.animate && nearestDistance > 6);
      const node = itemRefs.current.get(nearestId) ?? null;
...
function StormQuoteCard({
  ...
  const baseTextClasses =
    "max-w-3xl whitespace-pre-line text-center text-[1.45rem] font-sans font-semibold leading-[1.6] tracking-[0.01em] transition-all duration-500 ease-out sm:text-[1.85rem] md:text-[2.05rem]";
