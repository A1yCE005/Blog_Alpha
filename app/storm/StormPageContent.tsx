...
export function StormPageContent({ quotes }: StormPageContentProps) {
  ...
  const resolveNodeCenter = useCallback((node: HTMLElement, container: HTMLElement) => {
    let offset = 0;
    ...
    if (current === container) {
      return offset + node.offsetHeight / 2;
    }
    ...
    return (
      container.scrollTop + (nodeRect.top - containerRect.top) + nodeRect.height / 2
    );
  }, []);
  ...
  const animateContainerToNode = useCallback(
    (node: HTMLDivElement) => {
      ...
      const itemCenter = resolveNodeCenter(node, container);
    ...
    [cancelScrollAnimation, resolveNodeCenter]
  );
  ...
  const applyHighlightToCenter = useCallback(
    (options?: { animate?: boolean }) => {
      ...
      if (options?.animate) {
        ...
        if (node) {
          window.requestAnimationFrame(() => animateContainerToNode(node));
        }
      }
    },
    [animateContainerToNode, cancelScrollAnimation, resolveNodeCenter]
  );
  ...
  useEffect(() => {
    ...
    const focusSelected = () => {
      ...
      const maxScrollTop = Math.max(0, container.scrollHeight - container.clientHeight);
      const itemCenter = resolveNodeCenter(node, container);
      const nextTop = clamp(itemCenter - container.clientHeight / 2, 0, maxScrollTop);
    ...
    requestAnimationFrame(focusSelected);
  }, [applyHighlightToCenter, items, ready, resolveNodeCenter]);
...
function StormQuoteCard({
  ...
  const baseTextClasses =
    "max-w-3xl whitespace-pre-line text-center text-[1.45rem] font-bold leading-[1.55] tracking-[0.01em] transition-all duration-500 ease-out sm:text-[1.85rem] md:text-[2.05rem]";

  const highlightedClasses = highlighted
    ? "text-violet-300 drop-shadow-[0_0_48px_rgba(167,139,250,0.62)] drop-shadow-[0_0_120px_rgba(129,93,236,0.38)]"
    : depthActive
    ? "text-zinc-500/60 blur-[4px] opacity-35 saturate-[0.35] sm:blur-[6px] md:blur-[9px]"
    : "text-zinc-100/95";

  return (
    <article
      ref={setRefs}
      className={`flex justify-center px-6 py-8 transition-transform duration-500 ease-out will-change-transform transform-gpu ${
        highlighted ? "scale-[1.04]" : depthActive ? "scale-[0.92]" : "scale-[1]"
      }`}
    >
