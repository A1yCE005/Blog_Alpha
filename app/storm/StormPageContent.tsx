...
export function StormPageContent({ quotes }: StormPageContentProps) {
  ...
  const getCardState = (baseIndex: number) => {
    if (isIdle && baseIndex === focusedIndex) {
      return "focused";
    }
    if (isIdle) {
      return "idle";
  ...
  const renderQuote = (quote: string, extendedIndex: number) => {
    ...
    return (
      <li
        ...
        className="flex min-h-[32vh] items-center justify-center py-6 will-change-transform"
      >
        <blockquote
          className={`max-w-3xl text-center text-2xl font-semibold leading-snug transition-all duration-500 ease-out sm:text-3xl ${stateClasses}`}
        >
          {quote}
        </blockquote>
      </li>
    ...
    <>
      ...
      >
        <div className="mx-auto flex min-h-screen w-full max-w-4xl flex-col px-6 py-12 sm:px-10">
          <div className="flex justify-end pb-8 pt-16">
            <Link
              href="/"
          ...
          <div
            ref={containerRef}
            className={`relative flex-1 overflow-y-auto pb-20 ${styles.scrollArea}`}
