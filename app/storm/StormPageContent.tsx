...
const PAGE_FADE_DURATION = 300;
const ENTRY_DELAY_AFTER_FADE = 400;
const INITIAL_ENTRY_DELAY = PAGE_FADE_DURATION + ENTRY_DELAY_AFTER_FADE;
...
export function StormPageContent({ quotes }: StormPageContentProps) {
  ...
  const [entryDelayReady, setEntryDelayReady] = useState(false);
  ...
  useEffect(() => {
    if (!ready || items.length === 0 || initialHighlightRef.current || !entryDelayReady) {
      return;
    }
    ...
    requestAnimationFrame(focusSelected);
  }, [applyHighlightToCenter, entryDelayReady, items, ready, resolveTargetScrollTop]);
