...
export function StormPageContent({ quotes }: StormPageContentProps) {
  ...
  const postAnimationScrollGuardRef = useRef(false);
  ...
  const cancelScrollAnimation = useCallback(() => {
    ...
    suppressScrollHandlingRef.current = false;
    postAnimationScrollGuardRef.current = false;
  }, []);
  ...
  const animateContainerToNode = useCallback(
    (node: HTMLDivElement) => {
      ...
      const finish = () => {
        ...
        scrollAnimationFrameRef.current = null;
        postAnimationScrollGuardRef.current = true;
        requestAnimationFrame(() => {
          recenteringRef.current = false;
          suppressScrollHandlingRef.current = false;
        });
  ...
  const handleScroll = useCallback(
    (event: UIEvent<HTMLDivElement>) => {
      ...
      if (postAnimationScrollGuardRef.current) {
        postAnimationScrollGuardRef.current = false;
        return;
      }
