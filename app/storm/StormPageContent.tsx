...
export function StormPageContent({ quotes }: StormPageContentProps) {
  ...
  return (
    <>
      ...
      />
      <div
        className={`relative min-h-screen-dvh bg-black/80 pb-safe backdrop-blur-2xl supports-[backdrop-filter:blur(0)]:backdrop-blur-3xl page-fade-in transition-opacity duration-300 ease-out ${
          isTransitioning ? "pointer-events-none opacity-0" : "opacity-100"
        }`}
        onClick={handlePageClick}
      >
        <div className="mx-auto flex h-screen-dvh w-full max-w-4xl flex-col px-6 sm:px-12">
          {pool.length === 0 ? (
