...
export function StormPageContent({ quotes }: StormPageContentProps) {
  ...
  const applyHighlightToCenter = useCallback(
    (options?: { animate?: boolean; preferredId?: number }) => {
      const container = containerRef.current;
      if (!container) {
        cancelScrollAnimation();
        highlightedIdRef.current = null;
      ...
      const preferCandidate = (candidateId: number | null | undefined) => {
        if (candidateId === null || candidateId === undefined) {
          return;
        }

        const entry = distances.find((item) => item.id === candidateId);
        if (!entry) {
          return;
        }

        if (entry.distance <= nearestDistance + 1.5) {
          nearestId = candidateId;
          nearestDistance = entry.distance;
        }
      };

      preferCandidate(options?.preferredId);
      preferCandidate(highlightedIdRef.current);
  ...
  useEffect(() => {
    ...
    const scheduleInitialHighlight = () => {
      const focusSelected = () => {
        ...
        requestAnimationFrame(() => {
          suppressScrollHandlingRef.current = false;
          applyHighlightToCenter({ preferredId: selected.id });
        });
      };
