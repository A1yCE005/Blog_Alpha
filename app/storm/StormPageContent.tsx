...
export function StormPageContent({ quotes }: StormPageContentProps) {
  ...
  const [highlightSettled, setHighlightSettled] = useState(true);
  ...
  const animateContainerToNode = useCallback(
    (node: HTMLDivElement, onSettle?: () => void) => {
      const container = containerRef.current;
      if (!container) {
        if (onSettle) {
          onSettle();
              ...
              <div className="flex flex-col items-center gap-16 py-16 sm:gap-24">
                {items.map((item) => (
                  <StormQuoteCard
                    ...
                    settled={highlightSettled && highlightedId === item.id}
                  />
                ))}
              </div>
              {loading && (
...
function StormQuoteCard({
  ...
  const baseTextClasses =
    "max-w-3xl whitespace-pre-line text-center font-sans text-[1.45rem] font-bold leading-relaxed transition-[color,filter,opacity,transform] duration-500 ease-out sm:text-[1.85rem] md:text-[2.05rem]";

  const highlightedClasses = highlighted
    ? `text-violet-200 storm-quote-highlight${settled ? " is-settled" : ""}`
    : depthActive
    ? "text-zinc-500/60 blur-[4.5px] opacity-[0.32] saturate-[0.35]"
    : "text-zinc-100";
