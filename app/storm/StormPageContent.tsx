...
const scrollEase = (t: number) => {
  ...
  if (t < 0.5) {
    const scaled = t * 2;
    return (scaled * scaled * scaled * scaled * scaled) / 2;
  }

  const scaled = (1 - t) * 2;
  return 1 - (scaled * scaled * scaled * scaled * scaled) / 2;
};
...
export function StormPageContent({ quotes }: StormPageContentProps) {
  ...
  const desiredHighlightIdRef = useRef<number | null>(null);
  ...
  const stopScrollAnimation = useCallback(() => {
    ...
    scrollAnimationCompleteRef.current = null;
  }, []);
  ...
  const scrollContainerTo = useCallback(
    (
      ...
      suppressScrollHandlingRef.current = true;
      scrollAnimationCompleteRef.current = finish;
      const start = performance.now();
      const step = (now: number) => {
        const elapsed = now - start;
        const progress = Math.min(Math.max(elapsed / SCROLL_ANIMATION_DURATION, 0), 1);
        const next = currentTop + distance * scrollEase(progress);
        ...
        if (progress < 1) {
          ...
          requestAnimationFrame(() => {
            suppressScrollHandlingRef.current = false;
            const complete = scrollAnimationCompleteRef.current;
            scrollAnimationCompleteRef.current = null;
            complete?.();
          });
        }
  ...
  const applyHighlightToCenter = useCallback(
    (options?: { animate?: boolean }) => {
      ...
      desiredHighlightIdRef.current = nearestId;
      ...
      if (node) {
        ...
        scrollContainerTo(container, targetTop, animate, finalizeHighlight);
      } else {
        finalizeHighlight();
      }
    },
  ...
  const rerollItem = useCallback(
    (id: number) => {
      ...
      if (highlightedIdRef.current === id || desiredHighlightIdRef.current === id) {
        return;
      }
      setItems((current) =>
  ...
  return (
    <>
      ...
      />
      <div
        className={`relative min-h-screen bg-black/80 pb-[env(safe-area-inset-bottom)] backdrop-blur-2xl supports-[backdrop-filter:blur(0)]:backdrop-blur-3xl page-fade-in transition-opacity duration-300 ease-out [@supports(height:100dvh)]:min-h-[100dvh] ${
          isTransitioning ? "pointer-events-none opacity-0" : "opacity-100"
        }`}
        onClick={handlePageClick}
      >
        <div className="mx-auto flex h-screen min-h-screen w-full max-w-4xl flex-col px-6 pb-[env(safe-area-inset-bottom)] sm:px-12 [@supports(height:100dvh)]:h-[100dvh] [@supports(height:100dvh)]:min-h-[100dvh]">
          {pool.length === 0 ? (
            ...
            <div
              ...
              className="storm-scroll-container relative flex flex-1 flex-col overflow-y-auto px-2 sm:px-4"
              tabIndex={isInteractive ? 0 : -1}
            >
              <div className="flex flex-col items-center gap-14 pt-16 pb-[calc(4rem+env(safe-area-inset-bottom))] sm:gap-20">
                {items.map((item) => (
                  <StormQuoteCard
                    key={item.id}
                    item={item}
