...
export function StormPageContent({ quotes }: StormPageContentProps) {
  ...
  const highlightLockUntilRef = useRef(0);
  ...
  const cancelScrollAnimation = useCallback(() => {
    ...
    suppressScrollHandlingRef.current = false;
    highlightLockUntilRef.current = 0;
  }, []);
  ...
  const animateContainerToNode = useCallback(
    (node: HTMLDivElement) => {
      ...
      if (Math.abs(initialTarget - startTop) < 0.5) {
        ...
        highlightLockUntilRef.current = performance.now() + 180;
      ...
      suppressScrollHandlingRef.current = true;
      highlightLockUntilRef.current = performance.now() + SCROLL_ANIMATION_DURATION;
      ...
      const finish = () => {
        ...
        scrollAnimationFrameRef.current = null;
        highlightLockUntilRef.current = performance.now() + 220;
        requestAnimationFrame(() => {
          suppressScrollHandlingRef.current = false;
        });
      };
  ...
  const handleScroll = useCallback(
    (event: UIEvent<HTMLDivElement>) => {
      ...
      if (highlightLockUntilRef.current > performance.now()) {
        return;
      }
...
function StormQuoteCard({
  ...
  const baseTextClasses =
    "relative z-10 mx-auto max-w-3xl whitespace-pre-line text-center font-sans text-[1.4rem] font-bold leading-[1.85] tracking-wide transition-all duration-500 ease-out sm:text-[1.75rem] md:text-[1.95rem]";
