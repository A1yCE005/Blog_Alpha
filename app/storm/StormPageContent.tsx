...
export function StormPageContent({ quotes }: StormPageContentProps) {
  ...
  const computeItemOffset = useCallback((node: HTMLElement, container: HTMLElement) => {
  ...
  const animateContainerToNode = useCallback(
    (node: HTMLDivElement) => {
      ...
      const itemOffsetTop = computeItemOffset(node, container);
      const itemOffsetHeight = node.offsetHeight;
      const targetTop =
        itemOffsetTop + itemOffsetHeight / 2 - container.clientHeight / 2;
    ...
    [cancelScrollAnimation, computeItemOffset]
  );
...
function StormQuoteCard({
  ...
  const baseTextClasses =
    "max-w-3xl whitespace-pre-line text-center text-[1.5rem] font-bold leading-[1.55] tracking-[0.01em] transition-[color,filter] duration-500 ease-out sm:text-[1.85rem] md:text-[2.15rem]";

  const highlightedClasses = highlighted
    ? "text-violet-300 md:text-violet-200 drop-shadow-[0_0_60px_rgba(167,139,250,0.65)] md:drop-shadow-[0_0_140px_rgba(167,139,250,0.78)]"
    : depthActive
    ? "text-zinc-500/60 blur-[3px] md:blur-[6px] opacity-[0.35] saturate-[0.35] md:saturate-[0.25]"
    : "text-zinc-100";

  return (
    <article
      ref={setRefs}
      className={`flex justify-center px-6 py-8 transition-transform duration-500 ease-out will-change-transform ${
