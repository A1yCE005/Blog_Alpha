...
const INITIAL_HIGHLIGHT_DELAY = 400;
const INITIAL_HIGHLIGHT_TOTAL_DELAY = PAGE_FADE_DURATION + INITIAL_HIGHLIGHT_DELAY;
const HIGHLIGHT_STICKINESS = 6;
...
export function StormPageContent({ quotes }: StormPageContentProps) {
  ...
  const resolveNodeCenterWithinContainer = useCallback(
    (node: HTMLDivElement, container: HTMLDivElement) => {
      let top = node.offsetTop;
      let current = node.offsetParent as HTMLElement | null;
  ...
  const resolveTargetScrollTop = useCallback(
    (node: HTMLDivElement, container: HTMLDivElement) => {
      ...
      const nodeCenter = resolveNodeCenterWithinContainer(node, container);
      const target = nodeCenter - containerHeight / 2;
    ...
    [resolveNodeCenterWithinContainer]
  );
  ...
  const applyHighlightToCenter = useCallback(
    (options?: { animate?: boolean }) => {
      ...
      itemRefs.current.forEach((node, id) => {
        ...
        const itemCenter = resolveNodeCenterWithinContainer(node, container);
      ...
      const currentHighlightedId = highlightedIdRef.current;
      if (
        nearestId !== null &&
        currentHighlightedId !== null &&
        currentHighlightedId !== nearestId
      ) {
        const currentHighlightedNode = itemRefs.current.get(currentHighlightedId);
        if (currentHighlightedNode) {
          const currentDistance = Math.abs(
            resolveNodeCenterWithinContainer(currentHighlightedNode, container) -
              containerCenter
          );
          if (currentDistance - nearestDistance <= HIGHLIGHT_STICKINESS) {
            nearestId = currentHighlightedId;
  ...
  useEffect(() => {
    ...
    const scheduleInitialHighlight = () => {
      const focusSelected = () => {
...
function StormQuoteCard({
  ...
  const textSpanClasses =
    "storm-quote-body inline-block whitespace-pre-line transition-all duration-500 ease-out will-change-filter will-change-transform";
