...
export function PostCard({ post, href, className = "", size = "medium", ...linkProps }: PostCardProps) {
  const excerptClamp = size === "large" ? 6 : size === "medium" ? 4 : 3;
  const excerptTextSize = size === "large" ? "text-base" : size === "small" ? "text-xs" : "text-sm";
  const headingSize = size === "large" ? "text-3xl" : size === "small" ? "text-xl" : "text-2xl";
    ...
    >
      <article className="flex h-full flex-col gap-5 rounded-3xl border border-white/5 bg-gradient-to-b from-zinc-950/90 via-zinc-950/40 to-zinc-950/90 p-6 shadow-[0_30px_80px_-45px_rgba(167,139,250,0.55)] transition-transform duration-500 ease-out group-hover:-translate-y-1 group-hover:border-violet-400/60 group-hover:shadow-[0_45px_120px_-50px_rgba(167,139,250,0.65)]">
        ...
        <div className="flex flex-1 flex-col gap-3">
          <h3 className={`${headingSize} font-semibold text-zinc-50 transition-colors duration-300 group-hover:text-violet-200`}>
            {post.title}
          </h3>
          <p
            className={`${excerptTextSize} leading-relaxed text-zinc-400`}
            style={{
              display: "-webkit-box",
              WebkitLineClamp: excerptClamp,
        ...
        {post.tags.length > 0 && (
          <div className="mt-auto flex flex-wrap gap-2 pt-2 text-xs uppercase tracking-[0.2em] text-zinc-500">
            {post.tags.map((tag) => (
              <span
                key={tag}
