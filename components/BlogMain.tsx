...
export function BlogMain({ visible, posts }: BlogMainProps) {
  ...
  const showTransition = Boolean(transitioningSlug);
  ...
  return (
    <>
      ...
      <div
        aria-hidden="true"
        className={`pointer-events-none fixed inset-0 z-30 overflow-hidden transition-opacity duration-500 ease-in-out ${
          showTransition ? "opacity-100" : "opacity-0"
        }`}
      >
        <div
          className={`absolute inset-0 scale-110 transform-gpu bg-gradient-to-br from-violet-500/30 via-zinc-950 to-black transition-transform duration-500 ease-in-out ${
            showTransition ? "scale-100" : "scale-110"
          }`}
        />
        <div
          className={`absolute inset-0 opacity-0 transition-[opacity,transform] duration-500 ease-in-out ${
            showTransition ? "scale-125 opacity-100" : "scale-75"
          } bg-[radial-gradient(circle_at_center,_rgba(167,139,250,0.35),_rgba(12,10,24,0.92))]`}
        />
      </div>
    </>
  );
