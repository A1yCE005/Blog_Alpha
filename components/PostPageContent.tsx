...
import { PostPageTransitionControls } from "@/components/PostPageTransitionControls";
...
export function PostPageContent({ post, backHref = "/?view=blog" }: PostPageContentProps) {
  ...
  const containerId = `post-container-${post.slug}`;

  return (
    <div className="relative min-h-screen bg-black page-fade-in">
      <div id={containerId} className="post-transition-container">
        <div className="mx-auto w-full max-w-3xl px-6 py-20 sm:px-10">
          <PostPageTransitionControls
            backHref={backHref}
            resetKey={resetKey}
