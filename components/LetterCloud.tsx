...
const WordParticles = React.forwardRef<WordParticlesHandle, WPProps>(function WordParticles(props, ref) {
  ...
  React.useEffect(() => {
    ...
    function retargetToWord(newWord: string) {
      ...
      const particleCount = particles.length || baseParticleCount;
      ...
      const mismatch = targetsCount !== particleCount;
      const jitterRadius = gap * (mismatch ? 0.45 : 0.12);
      const ensureTarget = (idx: number) => {
        if (particleCount <= 0) {
        ...
        if (targetsCount === particleCount) {
          return targets[idx];
        }

        if (targetsCount > particleCount) {
          const scaled = (idx / particleCount) * targetsCount;
          const baseIdx = Math.min(targetsCount - 1, Math.floor(scaled));
          return targets[baseIdx];
        }

        const scaled = (idx / particleCount) * targetsCount;
        const baseIdx = Math.min(targetsCount - 1, Math.floor(scaled));
        const nextIdx = Math.min(targetsCount - 1, baseIdx + 1);
        const frac = scaled - baseIdx;
        const base = targets[baseIdx];
      ...
      if (particleCount > 0) {
        for (let i = 0; i < particleCount; i++) {
          const target = ensureTarget(i);
          const jitterX = (Math.random() - 0.5) * jitterRadius;
          const jitterY = (Math.random() - 0.5) * jitterRadius;
          particles[i].tx = target.x + jitterX;
          particles[i].ty = target.y + jitterY;
