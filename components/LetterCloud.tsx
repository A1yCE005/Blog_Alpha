  ...
  React.useEffect(() => {
    ...
    function startIdleScatter() {
      ...
      for (const p of particles) {
        ...
        const jitter = (Math.random() - 0.5) * idleGustJitter;
        p.sdx = Math.cos(ang);
        p.sdy = Math.sin(ang);
        p.vx = p.sdx * spd + jitter * 0.6;
        p.vy = p.sdy * spd + jitter * 0.6;
      }
    }
      ...
      if (!prefersReduced) {
        ...
        for (const p of particles) {
          if (phase === "drop") {
            ...
            const gatherEase = idleState === "gathering" ? 0.62 : 1;
            const kNow = (baseK + easeInOut(tLocal) * gainK) * gatherEase;
