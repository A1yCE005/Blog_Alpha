...
const WordParticles = React.forwardRef<WordParticlesHandle, WPProps>(function WordParticles(props, ref) {
  ...
  const referenceWordLength = React.useMemo(() => {
  ...
  React.useEffect(() => {
    ...
    const targetCache = new Map<string, { version: number; variants: Map<number, Array<{ x: number; y: number }>> }>();
    let cacheVersion = 0;
    ...
    function variantKey(count?: number) {
    ...
    function renderWordToTemp(text: string) {
      ...
      const referenceLength = Math.max(1, referenceWordLength);
      const lengthFactor = Math.max(1, referenceLength / Math.max(1, text.length));
      const baseSize = Math.max(12, Math.floor(Math.min(w, h) * CONFIG.wordScale));
      const baseTargetW = w * 0.58;
      const maxExtraWidth = w * 0.18;
      const targetW = Math.min(w - 24, baseTargetW + maxExtraWidth * Math.max(0, lengthFactor - 1));
      const baseMaxHeight = h * 0.46;
      const maxExtraHeight = h * 0.24;
      const targetMaxHeight = Math.min(h - 24, baseMaxHeight + maxExtraHeight * Math.min(lengthFactor - 1, 2));
      ...
      let size = Math.max(12, Math.floor(baseSize * lengthFactor));
      let { width: totalW, sp: spacingPx } = measureTotal(size);

      if (totalW > targetW) {
        const scale = targetW / Math.max(1e-3, totalW);
        size = Math.max(12, size * scale);
        ({ width: totalW, sp: spacingPx } = measureTotal(size));
      }

      const allowedHeight = Math.max(12, targetMaxHeight);
      if (size > allowedHeight) {
        size = allowedHeight;
        ({ width: totalW, sp: spacingPx } = measureTotal(size));
      }
    ...
    function getTargetsForWord(text: string, desiredCount?: number) {
      const key = variantKey(desiredCount);
      const cached = targetCache.get(text);
      if (cached && cached.version === cacheVersion) {
        const stored = cached.variants.get(key);
        if (stored) {
      ...
      const raw = collectTargets(desiredCount);

      let entry = targetCache.get(text);
      if (!entry || entry.version !== cacheVersion) {
        entry = { version: cacheVersion, variants: new Map() };
        targetCache.set(text, entry);
      }
      entry.variants.set(key, raw);
    ...
    function buildScene(text: string) {
      ...
      idleScatterElapsed = 0;
      idleGatherDelayLeft = 0;
      const targets = getTargetsForWord(text);
      const w = canvas.width / DPR;
      const h = canvas.height / DPR;
    ...
    function retargetToWord(newWord: string) {
      ...
      const desiredCount =
        particles.length > 0
          ...
          ? baseParticleCount
          : undefined;
      const targets = getTargetsForWord(newWord, desiredCount);
