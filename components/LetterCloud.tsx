...
const WordParticles = React.forwardRef<WordParticlesHandle, WPProps>(function WordParticles(props, ref) {
  ...
  React.useEffect(() => {
    ...
    type P = {
      ...
      sx: number; sy: number;   // 目标轻微偏移的恒定随机种子
      d?: number;                 // 错峰延迟（ms）
    ...
    const wordTargetsCache = new Map<string, Array<{ x: number; y: number }>>();
    let particlePoolSize = 0;

    function getTargetsForWord(text: string) {
      let cached = wordTargetsCache.get(text);
      if (!cached) {
        renderWordToTemp(text);
        cached = sampleTargetsFromTemp(gap);
        wordTargetsCache.set(text, cached);
      }
      return cached;
    }

    function rebuildPoolSize(words: string[]) {
      particlePoolSize = 0;
      for (const wText of words) {
        const count = getTargetsForWord(wText).length;
        if (count > particlePoolSize) particlePoolSize = count;
      }
      if (particlePoolSize === 0) {
        const base = getTargetsForWord(words[0] ?? currentWord).length;
        particlePoolSize = base || 1;
      }
    }

    function allocateTargets(baseTargets: Array<{ x: number; y: number }>, count: number) {
      if (count <= 0) return [] as Array<{ x: number; y: number }>;
      const assigned = new Array<{ x: number; y: number }>(count);
      ...
      if (len >= count) {
        ...
        let cursor = 0;
        for (let i = 0; i < count; i++) {
          assigned[i] = baseTargets[Math.min(len - 1, Math.floor(cursor))];
          cursor += step;
        }
      } else {
        const baseShare = Math.floor(count / len);
        let remainder = count % len;
        let idx = 0;
        for (let i = 0; i < len; i++) {
          const share = baseShare + (remainder > 0 ? 1 : 0);
          if (remainder > 0) remainder--;
          for (let j = 0; j < share && idx < count; j++, idx++) {
            assigned[idx] = baseTargets[i];
          }
        }
        for (let i = 0; i < count; i++) {
          if (!assigned[i]) {
            assigned[i] = baseTargets[i % len];
          }
        }
      }
      return assigned;
    ...
    function applyAssignedTargets(targets: Array<{ x: number; y: number }>) {
      const jitterScale = gap * 0.45;
      for (let i = 0; i < particles.length && i < targets.length; i++) {
        const p = particles[i];
        const base = targets[i];
        p.tx = base.x + p.sx * jitterScale;
        p.ty = base.y + p.sy * jitterScale;
        p.d = Math.random() * activeTransitionJitter;
    ...
    function buildScene(text: string) {
      ...
      const poolWords = Array.from(new Set([text, ...idleWords]));
      rebuildPoolSize(poolWords);
      const targetsForWord = allocateTargets(getTargetsForWord(text), particlePoolSize);
      ...
      for (let i = 0; i < particlePoolSize; i++) {
        const t = targetsForWord[i];
        pushOne(t);
      }

      applyAssignedTargets(targetsForWord);
    ...
    function retargetToWord(newWord: string) {
      ...
      const targets = allocateTargets(getTargetsForWord(newWord), particles.length);
