      ...
      if (!prefersReduced) {
        ...
        for (const p of particles) {
          if (phase === "drop") {
            ...
            const transitionRatioRaw = (baseTransitionMs || 1) / Math.max(1, activeTransitionMs);
            const transitionSpeedScale = Math.min(1.6, Math.max(0.28, Math.pow(transitionRatioRaw, 0.72)));
            const kNow = (baseK + easeInOut(tLocal) * gainK) * transitionSpeedScale;
