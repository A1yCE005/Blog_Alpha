...
const WordParticles = React.forwardRef<WordParticlesHandle, WPProps>(function WordParticles(props, ref) {
  ...
  React.useEffect(() => {
    ...
    const letterSpacingValue =
      typeof letterSpacing === "number" ? letterSpacing : CONFIG.letterSpacing ?? 0.06;
    const gapKey = gap.toFixed(2);
    const letterSpacingKey = letterSpacingValue.toFixed(3);
    const makeCacheKey = (text: string) =>
      `${canvas.width}x${canvas.height}:${DPR}:${gapKey}:${letterSpacingKey}:${text}`;
    ...
    const remapCache = new Map<string, Array<{ x: number; y: number }>>();
    let prewarmQueue: string[] = [];
    let prewarmIdleId: number | null = null;
    let prewarmTimeout: number | null = null;

    const cancelPrewarm = () => {
      if (prewarmIdleId != null && typeof (window as any).cancelIdleCallback === "function") {
        (window as any).cancelIdleCallback(prewarmIdleId);
      }
      if (prewarmTimeout != null) {
    ...
    const queuePrewarm = () => {
      ...
      const desired = particles.length || baseParticleCount;
      ...
      const pushWord = (w?: string) => {
        if (!w) return;
        if (seen.has(w) || w === currentWord) return;
        seen.add(w);
        prewarmQueue.push(w);
      };
      pushWord(CONFIG.word);
      idleWords.forEach((w) => pushWord(w));

      if (prewarmQueue.length === 0) return;

      const runNext = () => {
        if (prewarmQueue.length === 0) return;
        const nextWord = prewarmQueue.shift();
        if (!nextWord) return;
        renderWordToTemp(nextWord);
        const targets = collectTargets(nextWord, desired);
        const count = desired;
        const remapKey = `${makeCacheKey(nextWord)}:remap:${count}`;
        if (!remapCache.has(remapKey) || remapCache.get(remapKey)!.length < count) {
          remapCache.set(remapKey, remapTargets(targets, count));
        }
      ...
      const scheduleNext = () => {
        if (typeof (window as any).requestIdleCallback === "function") {
          prewarmIdleId = (window as any).requestIdleCallback(
          ...
          prewarmTimeout = window.setTimeout(() => {
            prewarmTimeout = null;
            runNext();
    ...
    function renderWordToTemp(text: string) {
      ...
      const factorLog = Math.log2(Math.max(1, inverseFactor));
      const targetW = w * Math.min(0.96, 0.86 + Math.min(0.08, factorLog * 0.05));
      ...
      const maxHeight = h * Math.min(0.96, 0.6 + Math.min(0.28, factorLog * 0.14));
      if (totalW > targetW) {
    ...
    function collectTargets(text: string, desiredCount?: number) {
      ...
      if (cached && cached.width === canvas.width && cached.height === canvas.height) {
        if (desiredCount == null || cached.targets.length >= desiredCount) {
    ...
    function buildScene(text: string) {
      ...
      const targets = collectTargets(text);
      const w = canvas.width / DPR;
      const h = canvas.height / DPR;
    ...
    function retargetToWord(newWord: string) {
      ...
      const desiredCount =
        particles.length > 0
          ...
          ? baseParticleCount
          : undefined;
      const targets = collectTargets(newWord, desiredCount);
      const currentCount = desiredCount ?? targets.length;
      const remapKey = `${makeCacheKey(newWord)}:remap:${currentCount}`;
      let mappedTargets = remapCache.get(remapKey);
      if (!mappedTargets || mappedTargets.length < currentCount) {
        mappedTargets = remapTargets(targets, currentCount);
        remapCache.set(remapKey, mappedTargets);
      }
