...
const WordParticles = React.forwardRef<WordParticlesHandle, WPProps>(function WordParticles(props, ref) {
  ...
  React.useEffect(() => {
    ...
    type TargetPoint = { x: number; y: number; ord: number };

    function computeHorizontalRanks(source: Array<{ x: number; y: number }>) {
      if (source.length <= 1) {
        return source.length === 1 ? [0.5] : [];
      }
      const entries = source
        .map((pt, idx) => ({ idx, x: pt.x, y: pt.y }))
        .sort((a, b) => (a.x === b.x ? a.y - b.y : a.x - b.x));
      const denom = Math.max(1, source.length - 1);
      const ranks = new Array(source.length).fill(0.5);
      for (let i = 0; i < entries.length; i++) {
        ranks[entries[i].idx] = i / denom;
      }
      return ranks;
    }

    function decorateTargets(source: Array<{ x: number; y: number }>): TargetPoint[] {
      if (source.length === 0) return [];
      const ranks = computeHorizontalRanks(source);
      return source.map((pt, idx) => ({ x: pt.x, y: pt.y, ord: ranks[idx] ?? 0.5 }));
    }
    ...
    function remapTargets(
      ...
      if (source.length === 0) {
        const fallback: TargetPoint[] = [];
        const cx = canvas.width / (DPR * 2);
        const cy = canvas.height / (DPR * 2);
        const denom = Math.max(1, desiredCount - 1);
      ...
      const withOrd = decorateTargets(source);
      const sorted = withOrd.slice().sort((a, b) => a.ord - b.ord);
      if (sorted.length === desiredCount) {
        return sorted;
      }

      const remapped: TargetPoint[] = [];
      if (sorted.length > desiredCount) {
        const step = sorted.length / desiredCount;
        ...
        for (let i = 0; i < desiredCount; i++, cursor += step) {
          ...
          remapped.push({ x: picked.x, y: picked.y, ord: picked.ord });
        }
        return remapped;
      }

      const repeats = Math.floor(desiredCount / sorted.length);
      const remainder = desiredCount % sorted.length;
      for (let r = 0; r < repeats; r++) {
        for (let i = 0; i < sorted.length; i++) {
          ...
          remapped.push({
            ...
            ord: base.ord + (Math.random() - 0.5) * 0.002
          });
        }
      }
      if (remainder > 0) {
        const step = sorted.length / remainder;
        let cursor = 0;
    ...
    function buildScene(text: string) {
      ...
      const baseRanks = computeHorizontalRanks(targets);

      const pushOne = (t: { x: number; y: number }, idx: number) => {
        ...
        particles.push({
          ...
          ord: baseRanks[idx] ?? idx / Math.max(1, need - 1)
        });
      };
    ...
    function retargetToWord(newWord: string) {
      ...
      const sortedTargets = mappedTargets
        .slice()
        .sort((a, b) => (a.ord ?? 0) - (b.ord ?? 0));
      ...
      // 写入新目标 & 刷新错峰与汇聚方向
      const assignCount = Math.min(sortedParticles.length, sortedTargets.length);
      for (let i = 0; i < assignCount; i++) {
        const target = sortedTargets[i];
        const particle = sortedParticles[i];
        particle.tx = target.x;
        particle.ty = target.y;
        particle.d = Math.random() * activeTransitionJitter;

        const angR = Math.random() * Math.PI * 2;
        const r0 = (CONFIG.funnelRadiusPx ?? 18) * (0.4 + Math.random() * 0.6);
        particle.hox = Math.cos(angR);
        particle.hoy = Math.sin(angR);
        particle.hrad = r0;
      }

      if (sortedParticles.length > assignCount) {
        const cx = canvas.width / (DPR * 2);
        const cy = canvas.height / (DPR * 2);
        for (let i = assignCount; i < sortedParticles.length; i++) {
          const particle = sortedParticles[i];
          const angR = Math.random() * Math.PI * 2;
          const r0 = (CONFIG.funnelRadiusPx ?? 18) * (0.4 + Math.random() * 0.6);
          particle.tx = cx;
