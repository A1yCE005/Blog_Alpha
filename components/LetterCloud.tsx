...
const WordParticles = React.forwardRef<WordParticlesHandle, WPProps>(function WordParticles(props, ref) {
  ...
  React.useEffect(() => {
    ...
    function sampleTargetsForCount(desiredCount: number) {
      if (!desiredCount || desiredCount <= 0) {
        return { targets: sampleTargetsFromTemp(gap), spacingUsed: gap };
      }

      let spacing = Math.max(0.2, gap);
      let targets = sampleTargetsFromTemp(spacing);
      let best = targets;
      let bestSpacing = spacing;
      let bestDiff = Math.abs(targets.length - desiredCount);

      const clampSpacing = (value: number) => {
        const minSpacing = Math.max(0.12, gap * 0.12);
        const maxSpacing = gap * 4.5;
        return Math.min(Math.max(value, minSpacing), maxSpacing);
      };
      ...
      for (let i = 0; i < maxIter; i++) {
        ...
        const adjust = Math.sqrt(Math.max(count, 1) / desiredCount);
        const nextSpacing = clampSpacing(spacing * adjust);

        if (!Number.isFinite(nextSpacing) || Math.abs(nextSpacing - spacing) < 1e-3) {
          break;
        }

        spacing = nextSpacing;
        targets = sampleTargetsFromTemp(spacing);
        const diff = Math.abs(targets.length - desiredCount);
        if (diff < bestDiff) {
          best = targets;
          bestSpacing = spacing;
          bestDiff = diff;
      ...
      return { targets: best, spacingUsed: bestSpacing };
    }
    ...
    function remapTargets(
      ...
      for (let r = 0; r < repeats; r++) {
        for (let i = 0; i < source.length; i++) {
          const base = source[i];
          remapped.push({
            x: base.x + (Math.random() - 0.5) * spacingHint * 0.35,
            y: base.y + (Math.random() - 0.5) * spacingHint * 0.35
          });
        }
      }
      if (remainder > 0) {
    ...
    function buildScene(text: string) {
      ...
      const { targets } = sampleTargetsForCount(0);
      const w = canvas.width / DPR;
      const h = canvas.height / DPR;
    ...
    function retargetToWord(newWord: string) {
      ...
      const currentCount = particles.length || baseParticleCount || 0;
      const { targets: remapSource, spacingUsed } = sampleTargetsForCount(currentCount || baseParticleCount || 0);
      const mappedTargets = remapTargets(remapSource, currentCount || remapSource.length, spacingUsed);
