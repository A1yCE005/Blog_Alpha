...
const WordParticles = React.forwardRef<WordParticlesHandle, WPProps>(function WordParticles(props, ref) {
  ...
  React.useEffect(() => {
    ...
    /** 将首轮汇聚的目标重排，使其尽量沿竖直方向就位 */
    function alignIntroTargets() {
      if (particles.length <= 1) return;
      const sortedTargets = particles
        .map((p) => ({ x: p.tx, y: p.ty }))
        .sort((a, b) => a.x - b.x);
      const sortedParticles = particles.slice().sort((a, b) => a.x - b.x);
      const count = Math.min(sortedTargets.length, sortedParticles.length);
    ...
    function step(ts?: number) {
      ...
      if (!prefersReduced) {
        ...
        if (!introSettled && phase === "morph" && morphElapsedMs === 0) {
          alignIntroTargets();
        }
        ...
        for (const p of particles) {
          if (phase === "drop") {
            ...
            const isIntroGather = !introSettled && idleState === "inactive";
            if (isIntroGather) {
              targetX = p.tx;
              targetY = p.ty;
            } else {
