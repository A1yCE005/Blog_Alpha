...
const WordParticles = React.forwardRef<WordParticlesHandle, WPProps>(function WordParticles(props, ref) {
  ...
  React.useEffect(() => {
    ...
    type P = {
      ...
      hrad?: number;              // 汇聚初始半径
      ix?: number; iy?: number;   // 首轮汇聚起始位置（用于柔和向上汇聚）
    };
    let particles: P[] = [];
    ...
    function retargetToWord(newWord: string) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        particles[i].ix = undefined;
        particles[i].iy = undefined;
      ...
      if (!prefersReduced) {
        ...
        for (const p of particles) {
          if (phase === "drop") {
            ...
            if (!introSettled) {
              const startX = p.ix ?? (p.ix = p.x);
              const startY = p.iy ?? (p.iy = p.y);
              const v = clamp01(tLocal);
              const lift = easeInOut(v);
              const lateral = easeInOut(Math.pow(v, 1.6));
              targetX = startX + (p.tx - startX) * lateral;
              targetY = startY + (p.ty - startY) * lift;
