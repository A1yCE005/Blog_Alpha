...
const WordParticles = React.forwardRef<WordParticlesHandle, WPProps>(function WordParticles(props, ref) {
  const {
    ...
    idleGatherSlowdown: idleGatherSlowdownProp,
    onWordChange
  } = props;
  ...
  const idleGatherSlowdown = Math.max(1, idleGatherSlowdownProp ?? CONFIG.idleGatherSlowdown ?? 1.35);
  ...
  React.useEffect(() => {
    ...
    type P = {
      ...
      hrad?: number;              // 汇聚初始半径
      svx?: number; svy?: number; // 待机散开时的基准漂移
    };
    let particles: P[] = [];
    ...
    const gatherCompleteMsBase = TRANS_DUR + TRANS_JIT + 260;
    ...
    function startIdleScatter() {
      ...
      for (const p of particles) {
        ...
        const drift = 0.45 + Math.random() * 0.85;
        p.svx = Math.cos(ang) * drift;
        p.svy = Math.sin(ang) * drift;
      }
    }
    ...
    function retargetToWord(newWord: string) {
      ...
      const targets = sampleTargetsFromTemp(gap);
      const need = targets.length;
      const jitterSpan = (idleState === "gathering" ? idleGatherSlowdown : 1) * TRANS_JIT;
      ...
      for (let i = 0; i < need; i++) {
        ...
        particles[i].d  = Math.random() * jitterSpan;
    ...
    function step(ts?: number) {
      ...
      if (!prefersReduced) {
        const gatherCompleteMs = (idleState === "gathering"
          ? gatherCompleteMsBase * idleGatherSlowdown
          : gatherCompleteMsBase);
        ...
        for (const p of particles) {
          if (phase === "drop") {
            ...
            const slowdown = idleState === "gathering" ? idleGatherSlowdown : 1;
            const transitionNow = (CONFIG.transitionMs || 1200) * slowdown;
            const tLocal = clamp01((morphElapsedMs - (p.d || 0)) / transitionNow);
            ...
            const gatherFactor = idleState === "gathering" ? 0.78 : 1;
            const kNow = baseK + easeInOut(tLocal) * gainK * gatherFactor;
            ...
            const driftX = (p.svx ?? 0) * idleAmbientDrift;
            const driftY = (p.svy ?? 0) * idleAmbientDrift;
            p.vx += (driftX + swirl * 0.08) * fscale;
            p.vy += (driftY + swirl * 0.05 + gravity * 0.012) * fscale;
            p.vx *= 0.984;
            p.vy *= 0.984;
            p.x += p.vx * fscale;
            p.y += p.vy * fscale;
    ...
    idleGustStrength, idleGustJitter, idleAmbientDrift, idleGatherSlowdown
  ]);
