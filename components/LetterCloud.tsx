...
const CONFIG = {
  ...
  idleGustStrength: 5.6,
  idleGustJitter: 2.2,
  idleAmbientDrift: 0.16,
  idleGatherTransitionMs: 2360
};
  ...
  React.useEffect(() => {
    ...
    function startIdleScatter() {
      ...
      for (const p of particles) {
        const baseSpd = idleGustStrength * (0.5 + Math.random() * 0.55);
      ...
      if (!prefersReduced) {
        ...
        for (const p of particles) {
          if (phase === "drop") {
            ...
            const idleEaseScale = idleState === "gathering" ? 0.72 : 1;
            const baseK = 0.04 * idleEaseScale;
            const gainK = (typeof morphK === "number" ? morphK : 0.14) * idleEaseScale;
            ...
            const swirlA = Math.sin((elapsedMs + p.tx * 7 + p.ty * 5) * 0.003) * 0.35;
            const swirlB = Math.cos((elapsedMs * 0.0018 + p.ty * 9 - p.tx * 6) * 0.004) * 0.28;
            ...
            p.vx += ((swirlA * 0.08) + (swirlB * 0.14) + jitterX) * fscale;
            p.vy += ((swirlB * 0.09) - (swirlA * 0.06) + jitterY + gravity * 0.008) * fscale;
            p.vx *= 0.984;
            p.vy *= 0.984;
            p.x += p.vx * fscale;
            p.y += p.vy * fscale;
