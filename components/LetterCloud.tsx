...
const WordParticles = React.forwardRef<WordParticlesHandle, WPProps>(function WordParticles(props, ref) {
  ...
  const referenceWord = React.useMemo(() => {
    if (CONFIG.word && CONFIG.word.length > 0) return CONFIG.word;
    if (idleWords.length > 0) return idleWords[0];
    return word;
  }, [idleWords, word]);
  ...
  React.useEffect(() => {
    ...
    const targetCache = new Map<string, Map<number, Array<{ x: number; y: number }>>>();
    ...
    let referenceMetrics: ReferenceMetrics = null;
    ...
    let exitElapsedMs = 0;
    let prewarmHandle: { type: "idle" | "timeout"; id: number } | null = null;
    ...
    function measureWordUnits(text: string) {
      const testSize = 100;
      tctx.font = `${CONFIG.fontWeight} ${testSize * DPR}px ${CONFIG.fontFamily}`;
      let width = 0;
      let maxAscent = 0;
      let maxDescent = 0;
      const spacingUnit = testSize * (letterSpacing ?? 0.06) * DPR;
      for (let i = 0; i < text.length; i++) {
        const metrics = tctx.measureText(text[i]);
        width += metrics.width;
        const ascent = metrics.actualBoundingBoxAscent ?? testSize * DPR * 0.74;
        const descent = metrics.actualBoundingBoxDescent ?? testSize * DPR * 0.26;
        if (ascent > maxAscent) maxAscent = ascent;
        if (descent > maxDescent) maxDescent = descent;
        if (i < text.length - 1) width += spacingUnit;
      }
      const widthUnit = width / (testSize * DPR);
      const heightUnit = (maxAscent + maxDescent) / (testSize * DPR);
      return {
        widthUnit: widthUnit > 0 ? widthUnit : 0.1,
        heightUnit: heightUnit > 0 ? heightUnit : 1
      };
    }

    function ensureReferenceLayout() {
      if (referenceMetrics) return;
      const refText = referenceWord && referenceWord.length > 0 ? referenceWord : currentWord;
      ensureTempSize();
      const w = canvas.width / DPR;
      const h = canvas.height / DPR;
      const minDim = Math.min(w, h);
      const targetWidth = Math.min(w * 0.88, Math.max(32, minDim * (CONFIG.wordScale ?? 0.6)));
      const maxHeightPx = Math.max(32, Math.min(h * 0.82, minDim * ((CONFIG.wordScale ?? 0.6) * 1.18)));
      const { widthUnit, heightUnit } = measureWordUnits(refText);
      let size = targetWidth / Math.max(widthUnit, 1e-3);
      if (heightUnit * size > maxHeightPx) {
        size = maxHeightPx / Math.max(heightUnit, 1e-3);
      }
      size = Math.max(14, size);
      const widthPx = widthUnit * size;
      const heightPx = heightUnit * size;
      referenceMetrics = {
        word: refText,
    ...
    function getLayout(text: string): LayoutInfo {
      ensureReferenceLayout();
      ...
      const targetWidth = Math.min(w * 0.88, Math.max(32, minDim * (CONFIG.wordScale ?? 0.6)));
      const maxHeightPx = Math.max(32, Math.min(h * 0.82, minDim * ((CONFIG.wordScale ?? 0.6) * 1.18)));
      const { widthUnit, heightUnit } = measureWordUnits(text);
      const baseArea = referenceMetrics?.area ?? (targetWidth * maxHeightPx);
      let size = Math.sqrt(baseArea / Math.max(widthUnit * heightUnit, 1e-6));
      const widthLimited = targetWidth / Math.max(widthUnit, 1e-6);
      const heightLimited = maxHeightPx / Math.max(heightUnit, 1e-6);
      size = Math.min(size, widthLimited, heightLimited);
      size = Math.max(14, size);
      const layout = {
        fontSize: size,
        width: widthUnit * size,
        height: heightUnit * size,
        widthUnit,
        heightUnit
      };
    ...
    function renderWordToTemp(text: string) {
      ...
      const fontPx = layout.fontSize * DPR;
      const spacingPx = layout.fontSize * (letterSpacing ?? 0.06);
      tctx.font = `${CONFIG.fontWeight} ${fontPx}px ${CONFIG.fontFamily}`;
    ...
    function collectTargetsForWord(text: string, desiredCount?: number) {
      const cacheKey = desiredCount && desiredCount > 0 ? desiredCount : 0;
      const cachedByCount = targetCache.get(text);
      const cachedTargets = cachedByCount?.get(cacheKey);
      if (cachedTargets) {
        return cachedTargets.map((pt) => ({ x: pt.x, y: pt.y }));
      }
      ...
      const stored = targets.map((pt) => ({ x: pt.x, y: pt.y }));
      if (!cachedByCount) {
        targetCache.set(text, new Map([[cacheKey, stored]]));
      } else {
        cachedByCount.set(cacheKey, stored);
      }
      return stored.map((pt) => ({ x: pt.x, y: pt.y }));
    }

    function schedulePrewarmTargets() {
      if (idleWords.length === 0) return;
      if (prewarmHandle) return;
      const desiredCount = baseParticleCount > 0 ? baseParticleCount : particles.length;
      if (!desiredCount || desiredCount <= 0) return;
      const pending = idleWords.filter((w) => w && w !== currentWord);
      if (pending.length === 0) return;

      const run = () => {
        for (const w of pending) {
          collectTargetsForWord(w, desiredCount);
        }
        prewarmHandle = null;
      };
      ...
      if (typeof ric === "function") {
        ...
        prewarmHandle = { type: "idle", id: id as number };
      } else {
        const id = window.setTimeout(run, 90) as unknown as number;
        prewarmHandle = { type: "timeout", id };
      }
    }
    ...
    function buildScene(text: string) {
      ...
      idleScatterElapsed = 0;
      idleGatherDelayLeft = 0;
      const targets = collectTargetsForWord(text);
      const w = canvas.width / DPR;
      const h = canvas.height / DPR;
    ...
    function retargetToWord(newWord: string) {
      ...
      const desiredCount =
        particles.length > 0
          ...
          ? baseParticleCount
          : undefined;
      const targets = collectTargetsForWord(newWord, desiredCount);
    ...
    return () => {
      ...
      ro.disconnect();
      if (prewarmHandle) {
        if (prewarmHandle.type === "idle") {
          (window as any).cancelIdleCallback?.(prewarmHandle.id);
        } else {
          window.clearTimeout(prewarmHandle.id);
        }
      }
