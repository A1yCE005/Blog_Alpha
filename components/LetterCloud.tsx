...
const WordParticles = React.forwardRef<WordParticlesHandle, WPProps>(function WordParticles(props, ref) {
  ...
  React.useEffect(() => {
    ...
    let lastLetterSpans: LetterSpan[] = [];
    let lastLetterSpacingPx = 0;
      ...
      for (let y = 0; y < temp.height; y += step) {
        for (let x = 0; x < temp.width; x += step) {
          ...
          if (img[idx] > alphaThreshold) {
            const xp = x / DPR + (Math.random() - 0.5) * (spacing * 0.6);
            const yp = y / DPR + (Math.random() - 0.5) * (spacing * 0.6);
            let letterIndex = 0;
            if (spans.length > 0) {
              let best = 0;
              ...
              for (let i = 0; i < spans.length; i++) {
                const span = spans[i];
                const dist = Math.abs(xp - span.center);
                if (dist < bestDist) {
                  bestDist = dist;
                  best = i;
                }
            ...
            const span = spans[letterIndex];
            const localWidth = span?.width ?? spacing;
            const normalized = span
              ? (xp - span.start) / Math.max(localWidth, spacing * 0.8)
              : 0;
            const order = letterIndex + clamp01(normalized) * 0.96 + (yp / Math.max(canvasHeight, 1)) * 0.002;
            targets.push({
              x: xp,
              y: yp,
              letterIndex,
    ...
    function remapTargets(
      ...
      if (source.length === desiredCount) {
        return source.map((s) => ({ x: s.x, y: s.y }));
      }

      const sorted = source.slice().sort((a, b) => a.order - b.order);
      const letterCount = spans.length || 1;
      const letterBuckets: TargetSample[][] = Array.from({ length: letterCount }, () => []);
      const letterCounts = new Array(letterCount).fill(0);
      for (const sample of sorted) {
        const idx = Math.max(0, Math.min(letterCount - 1, sample.letterIndex));
        letterBuckets[idx].push(sample);
        letterCounts[idx] += 1;
      }
      ...
      if (source.length > desiredCount) {
        ...
        for (let i = 0; i < desiredCount; i++, cursor += step) {
          const idx = Math.min(sorted.length - 1, Math.floor(cursor));
          const picked = sorted[idx];
          trimmed.push({ x: picked.x, y: picked.y });
        }
        return trimmed;
      }

      const total = letterCounts.reduce((sum, c) => sum + c, 0) || 1;
      const quotas = letterCounts.map((count, i) => {
        const exact = (count / total) * desiredCount;
        const base = Math.floor(exact);
        const frac = exact - base;
        return { idx: i, base, frac, exact };
      });
      let assigned = quotas.reduce((sum, q) => sum + q.base, 0);
      let remaining = desiredCount - assigned;
      const order = quotas
        .slice()
        .sort((a, b) => {
          if (b.frac !== a.frac) return b.frac - a.frac;
          return letterCounts[b.idx] - letterCounts[a.idx];
        });
      const targetCounts = quotas.map((q) => q.base);
      for (let i = 0; i < order.length && remaining > 0; i++, remaining--) {
        targetCounts[order[i].idx] += 1;
      }

      const remapped: Array<{ x: number; y: number }> = [];
      const jitterForLetter = (letterIdx: number) => {
        const span = spans[letterIdx];
        const baseWidth = span?.width ?? gap;
        const baseSpacing = lastLetterSpacingPx || gap;
        const rangeX = Math.min(baseWidth * 0.32, baseSpacing * 0.55, gap * 0.55);
        const rangeY = gap * 0.45;
        return {
          jx: (Math.random() - 0.5) * rangeX * 2,
          jy: (Math.random() - 0.5) * rangeY * 2
        };
      };

      for (let letterIdx = 0; letterIdx < letterCount; letterIdx++) {
        const share = targetCounts[letterIdx];
        if (share <= 0) continue;
        const bucket = letterBuckets[letterIdx];
        if (bucket.length === 0) continue;
        const repeats = Math.floor(share / bucket.length);
        const remainder = share % bucket.length;
        for (let r = 0; r < repeats; r++) {
          for (let i = 0; i < bucket.length; i++) {
            const base = bucket[i];
            const { jx, jy } = jitterForLetter(letterIdx);
            remapped.push({ x: base.x + jx, y: base.y + jy });
          }
        }
        if (remainder > 0) {
          const step = bucket.length / remainder;
          let cursor = 0.15;
          for (let i = 0; i < remainder; i++, cursor += step) {
            const idx = Math.min(bucket.length - 1, Math.floor(cursor));
      ...
      if (remapped.length < desiredCount) {
        const filler = sorted.length > 0 ? sorted : source;
        const needed = desiredCount - remapped.length;
        for (let i = 0; i < needed; i++) {
          const pick = filler[i % filler.length];
          remapped.push({ x: pick.x, y: pick.y });
        }
      }
    ...
    function retargetToWord(newWord: string) {
      ...
      const mappedTargets = remapTargets(targets, currentCount, lastLetterSpans);
