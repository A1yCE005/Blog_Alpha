...
const WordParticles = React.forwardRef<WordParticlesHandle, WPProps>(function WordParticles(props, ref) {
  ...
  React.useEffect(() => {
    ...
    function sampleTargetsFromTemp(spacing = gap, jitterScale = 0.45) {
      ...
      for (let y = 0; y < temp.height; y += step) {
        for (let x = 0; x < temp.width; x += step) {
          ...
          if (img[idx] > alphaThreshold) {
            targets.push({
              x: x / DPR + (Math.random() - 0.5) * (spacing * jitterScale),
              y: y / DPR + (Math.random() - 0.5) * (spacing * jitterScale)
            });
          }
        }
      }
    ...
    function retargetToWord(newWord: string) {
      ...
      let targets = sampleTargetsFromTemp(gap, 0.4);
      const currentCount = particles.length || baseParticleCount || targets.length;
      if (currentCount > 0 && targets.length < currentCount) {
        const currentRatio = targets.length / currentCount;
        const minSpacingPx = 1 / DPR;
        const minSpacing = Math.max(minSpacingPx, gap * 0.28);
        let spacing = Math.max(minSpacing, gap * Math.sqrt(Math.max(currentRatio, 0.0001)));
        spacing = Math.min(spacing, gap);
        let best = targets;
        let jitter = 0.22;
        for (let attempt = 0; attempt < 6 && best.length < currentCount; attempt++) {
          const next = sampleTargetsFromTemp(spacing, jitter);
          if (next.length > best.length) {
            best = next;
          }
          if (best.length >= currentCount) {
            break;
          }
          spacing = Math.max(minSpacing, spacing * 0.82);
          jitter = Math.min(0.45, jitter * 1.08);
        }
        if (best.length < currentCount && spacing > minSpacing) {
          const fallback = sampleTargetsFromTemp(minSpacing, Math.min(0.4, jitter));
