...
const WordParticles = React.forwardRef<WordParticlesHandle, WPProps>(function WordParticles(props, ref) {
  ...
  const referenceLetterCount = React.useMemo(() => {
    let baseline = Math.max(1, CONFIG.word?.length ?? 1);
    for (const idle of idleWords) {
      if (idle && idle.length > baseline) baseline = idle.length;
    }
    return baseline;
  }, [idleWords]);
  ...
  React.useEffect(() => {
    ...
    function renderWordToTemp(text: string) {
      ...
      // 根据 wordScale 与字数调整字号，使字母数越少时字号越大
      const letterCount = Math.max(1, text.length);
      const ratio = referenceLetterCount / letterCount;
      const baseSize = Math.floor(Math.min(w, h) * CONFIG.wordScale);
      let size = Math.max(12, baseSize * ratio);
      ...
      const targetW = Math.min(w * 0.92, w * 0.85 * Math.max(1, ratio));
      ...
      if (totalW > 0) {
        ...
        if (totalW > targetW) {
          const clamp = targetW / Math.max(1e-3, totalW);
          size *= clamp;
          ({ width: totalW, sp: spacingPx } = measureTotal(size));
        }
      }
    ...
    function collectTargets(desiredCount?: number) {
      ...
      if (!desiredCount || desiredCount <= 0 || targets.length === 0 || targets.length >= desiredCount) {
        return targets;
      }
      return remapTargets(targets, desiredCount);
    }
    ...
    function retargetToWord(newWord: string) {
      ...
      const mappedTargets =
        targets.length >= currentCount && currentCount > 0
          ? targets.slice(0, currentCount)
          : remapTargets(targets, currentCount);
