...
const WordParticles = React.forwardRef<WordParticlesHandle, WPProps>(function WordParticles(props, ref) {
  ...
  React.useEffect(() => {
    ...
    type P = {
      ...
      hrad?: number;              // 汇聚初始半径
      ix?: number; iy?: number;   // 首轮汇聚起点（用于直线就位）
    };
    let particles: P[] = [];
    ...
    function alignIntroTargets() {
      if (particles.length === 0) return;
      const targetsPool = particles.map((p) => ({ x: p.tx, y: p.ty }));
      const used = new Uint8Array(targetsPool.length);
      ...
      for (const entry of order) {
        ...
        for (let i = 0; i < targetsPool.length; i++) {
          if (used[i]) continue;
          const target = targetsPool[i];
          const dx = target.x - p.x;
          const dy = target.y - p.y;
          const score = dx * dx * 1.3 + dy * dy * 0.75;
          if (score < bestScore) {
            bestScore = score;
            bestIdx = i;
          }
        ...
        used[bestIdx] = 1;
        const chosen = targetsPool[bestIdx];
        p.tx = chosen.x;
        p.ty = chosen.y;
    ...
    function step(ts?: number) {
      ...
      if (!prefersReduced) {
        ...
        if (phase === "morph" && prevPhase !== "morph" && !introSettled && !introTargetsAligned) {
          alignIntroTargets();
          introTargetsAligned = true;
        }
        ...
        for (const p of particles) {
          if (phase === "drop") {
            ...
            if (!introSettled) {
              const startX = p.ix ?? p.x;
              const startY = p.iy ?? p.y;
              const direct = easeInOut(tLocal);
              targetX = startX + (p.tx - startX) * direct;
              targetY = startY + (p.ty - startY) * direct;
