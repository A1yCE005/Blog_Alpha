...
const WordParticles = React.forwardRef<WordParticlesHandle, WPProps>(function WordParticles(props, ref) {
  ...
  React.useEffect(() => {
    ...
    /** 首次构建场景（含首轮 drop） */
    let wordTargets = new Map<string, Array<{ x: number; y: number }>>();
    const seenWords = new Set<string>();

    const ensureTargets = (text: string) => {
      let cached = wordTargets.get(text);
      if (!cached) {
        renderWordToTemp(text);
        cached = sampleTargetsFromTemp(gap);
        wordTargets.set(text, cached);
      }
      return cached;
    };
    ...
    function buildScene(text: string) {
      ...
      idleGatherDelayLeft = 0;
      seenWords.add(text);
      for (const idleWord of idleWords) {
        seenWords.add(idleWord);
      }
      wordTargets = new Map();
      const baseWords = Array.from(seenWords);
      const precomputed: Record<string, Array<{ x: number; y: number }>> = {};
      let maxCount = 0;
      for (const baseWord of baseWords) {
        const t = ensureTargets(baseWord);
        precomputed[baseWord] = t;
        if (t.length > maxCount) maxCount = t.length;
      }

      const targets = precomputed[text] ?? [];
      const w = canvas.width / DPR;
      const h = canvas.height / DPR;

      // 初始化粒子
      const previousChars = particles.map((p) => p.c);
      particles.length = 0;
      for (let i = 0; i < maxCount; i++) {
        const t = targets[i];
        const active = Boolean(t);
        const char = previousChars[i] ?? glyphs[(Math.random() * glyphs.length) | 0];
        ...
        if (active && t) {
          ...
          const parkJitter = gap * 1.6;
          const px = hubX + (Math.random() - 0.5) * parkJitter;
          const py = hubY + (Math.random() - 0.5) * parkJitter;
    ...
    function retargetToWord(newWord: string) {
      ...
      const targets = ensureTargets(newWord);
      ...
      if (particles.length < need) {
        const prevChars = particles.map((p) => p.c);
        for (let i = particles.length; i < need; i++) {
          const t = targets[i];
          const angR = Math.random() * Math.PI * 2;
          const r0 = (CONFIG.funnelRadiusPx ?? 18) * (0.4 + Math.random() * 0.6);
          const char = prevChars[i] ?? glyphs[(Math.random() * glyphs.length) | 0];
          particles.push({
            x: t.x + (Math.random() - 0.5) * gap,
            y: t.y + (Math.random() - 0.5) * gap,
            vx: 0,
            vy: 0,
            tx: t.x,
            ty: t.y,
